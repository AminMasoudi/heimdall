...
class AESEncryption:
    ...
    def __g(self, word: bytes, rc:int)->bytes:
        ...
        new_bytes = (new_bytes[0] ^ rc).to_bytes() + new_bytes[2:]
    ...
    def decrypt_block(self, block:bytes)->bytes:
        ...
        for round in range(10, 0, -1):
            ...
            table = self.__add_round_key(table, self.__round_keys[round*4 - 4: round*4])
            if round != 1:
    ...
    def __inv_shift_rows(self, table:list[bytes])-> list[bytes]:
        ...
        for counter in range(4):
            string = b"".join(
                [table[(i + counter) % 4][j:j+1] for j, i in enumerate(range(0, -4, -1))]
            )
            new_table.append(string)
        return new_table
